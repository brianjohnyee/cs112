...
let interp_stmt (stmt : Absyn.stmt) = match stmt with
    | Dim (ident, expr) -> 
        (Hashtbl.add Tables.array_table ident (Array.make 
        (int_of_float(eval_expr expr)) 0.))
    | Let (memref, expr) -> 
        (match memref with
        | Variable ident -> 
        ...
        | Arrayref (ident, exprr) ->
            ...
            (int_of_float(eval_expr exprr)-1) (eval_expr expr) )
        )
    | Goto label -> 
        ...
        (match stmt with
            ...
            | If (expr, label) ->
                (match expr with
                    ...
                    | Binary (oper,expr1,expr2)->
                        if (Hashtbl.find Tables.boolean_table oper) 
